# backend/scripts/ingest_google_places.py
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest places from Google Places Text Search into Postgres."
    )
    parser.add_argument("query", help='Search query, e.g. "museums in Detroit"')
    parser.add_argument(
        "--max-results",
        type=int,
        default=int(os.getenv("INGEST_MAX_RESULTS", "20")),
    )
    parser.add_argument(
        "--sleep",
        type=float,
    ...
    parser.add_argument(
        "--paged",
        action="store_true",
        help="Fetch up to 2 pages (may cost more).",
    )
    ...
    for raw in raw_results:
        ...
        # Save raw payload for inspection + future enrichment
        upsert_place_external_id(
            place_id=saved["id"],
            source="google",
            external_id=saved.get("google_place_id"),
            source_url=saved.get("google_maps_url"),
            raw_payload=raw,
        )
        ...
        print(
            f"Upserted: {saved['name']} "
            f"| city={saved.get('city')} region={saved.get('region')} "
